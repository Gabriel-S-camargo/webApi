...
function Modal({ paciente }: ModalProps) {
  ...
  function handleChange(e: SimpleChangeEvent) {
    ...
    if (name === "dataNascimento") {
      setForm((prev) => ({
        ...prev,
        dataNascimento: value as string,
      }));
      return;
    }
  ...
  async function handleConfirm() {
    ...
    if (isEditing) {
      updatePaciente.mutate(
        { id: paciente!.id, payload: apiPayload },
        { onSuccess, onError }
      );
    } else {
      createPaciente.mutate(apiPayload, { onSuccess, onError });
    }
  }
  ...
  return (
    ...
    >
      {(manualLoading || isSubmitting) && (
        <LoadingOverlay text="Salvando paciente..." />
      )}
